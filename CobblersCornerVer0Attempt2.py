...
with tab1:
    ...
    if st.button("🚀 Process & Calculate Everything"):
        if not (uploaded_magic and uploaded_bracket and uploaded_members):
            ...
            # Clean Columns (Strip out whitespace)
            ...
            # Smart helper to find any column containing "email" dynamically
            def find_email_col(columns_list):
                for c in columns_list:
                    if 'email' in str(c).lower():
                        return c
                return columns_list[0] # Default fallback

            # Dynamically identify correct column indices
            magic_email_col = find_email_col(magic_df.columns)
            member_email_col = find_email_col(member_df.columns)
            
            # 1. Map Member Database via Clean Email List
            valid_member_emails = set(member_df[member_email_col].dropna().astype(str).str.strip().str.lower().unique())
            
            # 2. Extract Tournament Placements from Bracket file
            player_placements = {}
            # Dynamically look across any column containing "email" inside the bracket standings file
            bracket_email_cols = [c for c in bracket_df.columns if 'email' in c.lower()]
            
            # Fallback to defaults if specific string wasn't parsed
            if not bracket_email_cols:
                bracket_email_cols = ['PlayerEmail1', 'PlayerEmail2', 'PlayerEmail3', 'PlayerEmail4']

            for _, row in bracket_df.iterrows():
                try:
                    place = int(row['Place'])
                except:
                    continue # Skip spacer/header rows if they exist in bracket csv
                
                for email_col in bracket_email_cols:
            ...
            processed['Email_Lower'] = processed[magic_email_col].fillna('').astype(str).str.strip().str.lower()
            ...
            # Ensure output has standard 'Email' header for Power BI
            if 'Email' in processed.columns and magic_email_col != 'Email':
                processed['Email'] = processed[magic_email_col]

...
if app_mode == "📊 Weekly Data Stream Processing Suite":
    ...
    st.info("💡 **Drop All Files Together:** Upload your raw ScoreMagic stats, Bracket Standings, and Player Rosters simultaneously. They are securely matched using internal data keys.")
    ...
    if uploaded_files:
        ...
        st.success(f"Successfully staged `{len(stats_list)}` Stat Groups, `{len(brackets_by_game)}` Bracket Maps, and `{len(rosters_by_game)}` Roster Lookups in memory.")
        st.write("---")

        # --- PRE-PROCESS DATA STORAGE FOR INDEPENDENT PIPELINE BLOCKS ---

...
if app_mode == "📊 Preset Timeframe Stats Generator":
    ...
    if st.button("🚀 Process Streams & Compile 4-Output Matrix"):
        if not scoremagic_dfs:
            ...
            post_text += f"🏆 **NIGHTLY TOP 10 LEADERBOARD (Min. 10 Rounds Thrown):**\n"
            post_text += "----------------------------------------------------\n"
            for rank, (_, row) in enumerate(top_10_leaders.iterrows(), 1):
                post_text += f"{rank:02d}. 👤 **{row['Name Check']}** — {row['PPR']:.2f} PPR | **{int(row['Total'])} Total Pts** ({row['Assigned Tier Category']})\n"
            
            post_text += f"\n👑 **THE EFFICIENCY SECTOR RADARS:**\n"
            post_text += "----------------------------------------------------\n"
            post_text += f"🔥 **Highest Overall PPR Execution:** {df_output.sort_values(by='PPR', ascending=False).iloc[0]['Name Check']} ({df_output.sort_values(by='PPR', ascending=False).iloc[0]['PPR']:.2f} Avg PPR)\n"
            post_text += f"🎯 **4-Bagger Streak King:** {df_output.sort_values(by='4Bagger %', ascending=False).iloc[0]['Name Check']} ({int(df_output.sort_values(by='4Bagger %', ascending=False).iloc[0]['4Bagger %'])}% max value rate)\n"
            post_text += f"❌ **Clean Board Execution (Lowest OFF%):** {df_output.sort_values(by='OFF %', ascending=True).iloc[0]['Name Check']} (Only missing the board {df_output.sort_values(by='OFF %', ascending=True).iloc[0]['OFF %']:.1f}% of throws)\n\n"
    ...
    if master_file:
        ...
        if timeframe_scope != "Year-Over-Year Comparison (2025 vs 2026)":
            if not filtered_df.empty:
                ...
                player_stats = filtered_df.groupby('Name Check').agg({
                    'Total': 'sum',
                    'Rounds': 'sum',
                    'PPR': 'mean',
                    'OFF %': 'mean',
                ...
                with tab2:
                    ...
                    breakout_list = []
                    for player, group in filtered_df.groupby('Name Check'):
                    ...
                    for rank, (_, r) in enumerate(player_stats.sort_values(by='Total', ascending=False).head(5).iterrows(), 1):
                        report_text += f"{rank}. **{r['Name Check']}** — {int(r['Total'])} Total Points ({r['PPR']:.2f} Historical Avg PPR)\n"
                    
                    report_text += f"\n🎯 **STAMINA & WORKHORSE MILESTONES:**\n"
                    report_text += "----------------------------------------------------\n"
                    report_text += f"• **Most Dedicated Workhorse:** {top_round_workhorse['Name Check']} throwing a massive **{int(top_round_workhorse['Rounds'])} rounds** over this period!\n"
        ...
        # --- YEAR-OVER-YEAR COMPARISON CALCULATION MODULE ---
            ...
            stats_2025 = df_2025.groupby('Name Check').agg({'Rounds': 'sum', 'PPR': 'mean', 'OFF %': 'mean'}).reset_index()
            stats_2026 = df_2026.groupby('Name Check').agg({'Rounds': 'sum', 'PPR': 'mean', 'OFF %': 'mean'}).reset_index()
            
            yoy_merged = pd.merge(stats_2025, stats_2026, on='Name Check', suffixes=('_2025', '_2026'))
            ...
            if not yoy_qualified.empty:
                ...
                display_yoy = yoy_qualified[['Name Check', 'PPR_2025', 'PPR_2026', 'PPR Delta', 'OFF Delta']].sort_values(by='PPR Delta', ascending=False)
                st.dataframe(display_yoy, use_container_width=True)
                
                # Generate Year Over Year Post Block
                ...
                for idx, (_, row) in enumerate(display_yoy.head(5).iterrows(), 1):
                    yoy_text += f"{idx:02d}. 🏆 **{row['Name Check']}**\n"
                ...
                yoy_text += "====================================================\n"
                yoy_text += "The development is real! Keep putting in those
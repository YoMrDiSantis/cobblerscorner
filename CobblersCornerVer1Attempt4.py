...
def get_dynamic_date_meta(week_code, target_year):
    ...
    # Accurate grouping tracking matching historical layout structures
    if wk_num <= 8:
        month_str = "January"
        seq_num = "101"
    elif wk_num <= 16:
        ...
        if not scoremagic_dfs:
            ...
            for df_file in scoremagic_dfs:
                ...
                for _, row in df_file.iterrows():
                    ...
                    # Core Total Bags calculation base (4 bags thrown per standard round)
                    total_bags = rounds * 4
                    ...
                    # Extract safe percentages
                    four_in_pct = clean_pct(row.get('4IN %', 0))
                    in_pct = clean_pct(row.get('IN %', 0))
                    on_pct = clean_pct(row.get('ON %', 0))
                    off_pct = clean_pct(row.get('OFF %', 0))
                    
                    # --- COMPILING EXACT WHOLE BAG INTEGERS REQUIRED BY SCHEMA ---
                    # 4IN = whole number of rounds where all 4 bags landed in the hole
                    raw_4in = int(pd.to_numeric(row.get('4IN', 0), errors='coerce') or round((four_in_pct / 100.0) * rounds))
                    # IN/ON/OFF = exact individual bag totals calculation logic
                    calculated_in = int(round((in_pct / 100.0) * total_bags))
                    calculated_on = int(round((on_pct / 100.0) * total_bags))
                    calculated_off = int(round((off_pct / 100.0) * total_bags))
                    ...
                    # Append mapped structured keys matching columns exactly
                    compiled_rows.append({
                        ...
                        'OPP PPR': opp_ppr,
                        '4IN': raw_4in,
                        '4IN %': int(round(four_in_pct)),
                        'IN': calculated_in,
                        'IN %': int(round(in_pct)),
                        'ON': calculated_on,
                        'ON %': int(round(on_pct)),
                        'OFF': calculated_off,
                        'OFF %': int(round(off_pct)),
                        'Rounds': rounds,
                        ...
                        '': "",  # CRITICAL STRUCTURAL SEPARATOR TO MATCH THE BLANK COLUMN 28 HEADERS EXPLICITLY
            ...
            # --- TRUE EXCEL COMPRESSION WORKBOOK EXPORTER ---
...
# ==============================================================================
    ...
    if master_file:
        try:
            # Parse the exact tab expected by your configuration structure

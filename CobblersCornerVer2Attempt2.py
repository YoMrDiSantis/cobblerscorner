...
# Increase CSV parser field size limit to prevent crash on massive cells/strings
    ...
    if uploaded_files:
        ...
        for f in uploaded_files:
            try:
                ...
                st.error(f"Error parsing file metadata for {f.name}: {ex}. This stream item was bypassed securely.")
        ...
        if st.button("🚀 Execute Processing Engine & Compile Outputs", key="run_main_engine"):
            ...
            if all_processed_records:
                ...
                if potw_records:
                    ...
                    st.info("No Delaware Cornhole members identified to assign weekly points.")

                st.write("---")
                st.header("🎨 Pipeline 3: Canva Clipboard Text Generation Engine")
                canva_raw_output = [f"✨ DELAWARE CORNHOLE WEEKLY REPORT • {selected_week_code} ✨\n", "🏆 TOURNAMENT PODIUM CHAMPIONS"]
                for summary in canva_brackets_summary: canva_raw_output.append(summary)
                if canva_top_pprs:
                ...
                st.error("Data extraction yielded blank frames. Verify your source file columns.")
...
# ==============================================================================
    ...
    historical_file = st.file_uploader("📥 Ingest Master Historical Record Sheet (.xlsx Format)", type=["xlsx"])
    if historical_file:
        try:
